...
void setupTCInterrupts(void)
{
	...
	TIM_TimeBaseStructure.TIM_Period = 124;				//频率为8k = 125us
	TIM_TimeBaseStructure.TIM_Prescaler = (72-1);	//72分频，频率为1MHz
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseInit(TIM1, &TIM_TimeBaseStructure);
	
	TIM_ClearITPendingBit(TIM1, TIM_IT_Update); //清除更新中断
	TIM_SetCounter(TIM1, 0); //设置计数初值为0
...
void enableTCInterrupts(void)
{
	...
	TIM_ClearITPendingBit(TIM1, TIM_IT_Update); //清除更新中断
	TIM_ITConfig(TIM1, TIM_IT_Update, ENABLE); //使能TIM1的更新中断
}

void disableTCInterrupts(void)
{
	TC1_ISR_Enabled = false;
	TIM_ITConfig(TIM1, TIM_IT_Update, DISABLE); //失能TIM1的更新中断
	TIM_ClearITPendingBit(TIM1, TIM_IT_Update); //清除更新中断
}
...
void StepperCtrl_setLocationFromEncoder(void)
{
	...
	if (CalibrationTable_calValid())
	{
		...
		//we need to set our numSteps( numSteps = ((a/65536)*200)*16 ) 一圈(0-65535)是(200步 * 细分)
		numSteps = (int64_t)DIVIDE_WITH_ROUND(((uint32_t)a * (motorParams.fullStepsPerRotation >> 3)), (fullMicrosteps >> 3));  //(x/y)+0.5
		//numSteps = a*25/512 + 0.5  = a*3200/65536  (x= a*25,y =512 )
		currentLocation = (int64_t)a; //保存当前位置
	}
...
int64_t StepperCtrl_getDesiredLocation(void)
{
	...
	ret = (int64_t)DIVIDE_WITH_ROUND((numSteps * (fullMicrosteps >> 2)), (motorParams.fullStepsPerRotation >> 2));
		//ret（65536） = numSteps（3200） * 4096 / 200   (65536/16 = 4096 对应 3200/16 = 200) 
	return ret;
}
...
bool StepperCtrl_calibrateEncoder(void)
{
	...
	bool done = false; //校正完成标志位
	bool feedback = enableFeedback; //缓存状态
	bool state = TC1_ISR_Enabled;
	disableTCInterrupts(); //关闭更新中断
	...
	A4950_move(0, motorParams.currentMa); //相回零
	delay_ms(1200);
	
	while(!done) //Starting calibration
	{		
		delay_ms(180);
		mean = StepperCtrl_sampleMeanEncoder(202);	//取202个值计算平均值
		...
		j++;		
		if(j >= CALIBRATION_TABLE_SIZE) //CALIBRATION_TABLE_SIZE == 256 把一圈分成 256 个刻度
		{
			done = true;
		}
	}	
...
float StepperCtrl_measureStepSize(void)
{
	...
	A4950_move(0,motorParams.currentMa); //this puts stepper motor at stepAngle of zero //motorReset
	delay_ms(1200);
...
//			x=(uint32_t)(GetMotorVoltage()*1000.0);
	...
	fullMicrosteps = (uint32_t)(ANGLE_STEPS / systemParams.microsteps);			// 即 65536/16 = 4094 对应 3200/16 = 200步
	fullStep = (int32_t)(ANGLE_STEPS / motorParams.fullStepsPerRotation);		//每一整步对应的角度值 65536/200 = 327  (360度/200 = 1.8度） 
																																					// 即 角度值327 对应 1.8度
	...
	enableFeedback = true;//使能反馈
	setupTCInterrupts(); 	//启动TC1
	enableTCInterrupts();	//使能TC1中断
...
bool StepperCtrl_processFeedback(void)
{
	...
	step_cnt++;
	//if(step_cnt == 3200)
	if(fabs(desiredLoc-currentLoc) < 350)
	{
		step_cnt = 0;
	}
...
bool StepperCtrl_simpleFeedback(int64_t desiredLoc, int64_t currentLoc)
{
	...
	if(enableFeedback)
	{
		...
		iTerm += (sPID.Ki * error); //积分项		
		x = (int32_t)(iTerm >> 10);
																						
		if(fastAbs(x) > fullStep) //积分限幅
		{
			if(x > fullStep)
			{
				x = fullStep;
		...
		//位置式离散PID公式：Kp*e(k) + Ki*∑e(k) + Kd[e（k）-e(k-1)]
		//增量式离散PD公式：Kp*e(k) + Kd[e（k）-e(k-1)]
		u = ((sPID.Kp * error) >> 10) + x + ((sPID.Kd * (error - lastError)) >> 10);	//增量u
...
uint16_t StepperCtrl_maxCalibrationError(void)
{
	uint16_t j = 0;
	int32_t steps = 0;
	uint16_t microSteps = systemParams.microsteps; //缓存细分
	
	uint16_t mean;
	uint16_t desiredAngle; //角度值
	uint16_t cal;
	
	int32_t dist; //误差
	uint16_t maxError = 0; //最大误差
	
	bool done = false; //测试完成标志位
	bool feedback = enableFeedback; //缓存状态
	bool state = TC1_ISR_Enabled;
	disableTCInterrupts(); //关闭更新中断
	disableINPUTInterrupts();
	...
	while(!done)
	{
		...
		cal = CalibrationTable_getCal(desiredAngle); //由角度值换算出编码器值(0-32767)
		...
		if(fastAbs(dist) > maxError) //得到最大误差值
		{
