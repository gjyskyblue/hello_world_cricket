...
menuItem_t MenuCal[] = {	//Calibrate菜单
		{"Calibrate", menuCalibrate, NULL},
		{ "", NULL, NULL}
};
...
void MKS_begin(void)
{
	...
	nonvolatile_begin(); //求解NVM_address
...
#ifndef MKS_SERVO42B
	display_show("MKS", "Servo57B", VERSON, ""); //显示57LOGO
#else
	display_show("MKS", "Servo42B", VERSON, ""); //显示42LOGO
#endif
	...
	while(STEPCTRL_NO_ERROR != stepCtrlError)
	{
		...
		if(STEPCTRL_NO_CAL == stepCtrlError)
		{
			...
			StepperCtrl_calibrateEncoder();	//skyblue_add
...
{
	if(NVM->SystemParams.errorPinMode == ERROR_PIN_MODE_ENABLE) //高电平有效
	{
		enableState = (bool)GPIO_ReadInputDataBit(PIN_INPUT, PIN_INPUT_ENABLE); //read our enable pin
	}
	if(NVM->SystemParams.errorPinMode == ERROR_PIN_MODE_ACTIVE_LOW_ENABLE) //默认低电平有效
	{	
		enableState = !(bool)GPIO_ReadInputDataBit(PIN_INPUT, PIN_INPUT_ENABLE); //read our enable pin
	}
}
