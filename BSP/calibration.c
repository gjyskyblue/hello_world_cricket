...
bool CalibrationTable_updateTableValue(uint16_t index, uint16_t value)
{
	...
	CalData[index].error = CALIBRATION_STEPS / CALIBRATION_TABLE_SIZE;	//误差
	return true;
}
...
uint16_t CalibrationTable_reverseLookup(uint16_t encoderAngle)
{
	...
	while(i < CALIBRATION_TABLE_SIZE)
	{
		...
		{
			//x1=a1  y1=b1=    i*(65536/200)+0.5   		//ANGLE_STEPS=65536,CALIBRATION_TABLE_SIZE=256			
			//x2=a2  y2=b2=(i+1)*(65536/200)+0.5   			
			//x=encoderAngle
			b1 = (int32_t)DIVIDE_WITH_ROUND(((int64_t)i     * ANGLE_STEPS),CALIBRATION_TABLE_SIZE);  //(x/y)+0.5
			b1t = i*256+0.5;
			b2 = (int32_t)DIVIDE_WITH_ROUND(((int64_t)(i+1) * ANGLE_STEPS),CALIBRATION_TABLE_SIZE);
			b2t = (i+1)*256+0.5;
			y = interp(a1,b1,a2,b2,x); //(y-b1)/(x-a1) = (b2-b1)/(a2-a1)
			yt = b1 + (b2-b1)*(x-a1)/(a2-a1);
			return y;										
		}
		i++;
	}
...
//	printf("data.MAX %d\n\r",data.MAX);
	...
	nvmWriteCalTable(&data); //存储CalTable

	CalibrationTable_createFastCal(); //创建FastCalTable
}
...
void CalibrationTable_createFastCal(void)
{
	uint32_t i,j;
	uint16_t checkSum = 0;
	uint16_t data[512]; //1K字节缓存，启动文件栈空间大小至少调整为2K
	for (i=0,j=0; i < 16384; i++) //磁编刻度0~16383 对应 电机位置刻度 0~63335 
	{															//
		uint16_t x = 0;
		...
		{			//flash的第32页到63页用来存储FastCal数据
			Flash_ProgramPage( (FLASH_PAGE32_ADDR + (uint32_t)(((i + 1) * 2) - FLASH_PAGE_SIZE)), data, FLASH_ROW_SIZE );
			j=0;
		}
		checkSum += x;
	}
	Flash_ProgramSize(FLASH_checkSum_ADDR, &checkSum, 1);	//存储checkSum
...
void CalibrationTable_init(void)
{
	...
	{
		CalibrationTable_loadFromFlash();	//已经有cal数据
...
{
	uint16_t i;
	uint16_t checkSum = 0; //checkSum类型必须与CalibrationTable_createFastCal函数中的一致
	bool NonZero = false;
	for(i=0; i < 16384; i++)
	{
